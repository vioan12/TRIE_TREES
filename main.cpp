...
void DisplayTrie2(TrieNode *trie);

int sw;
void test(TrieNode *trie);
...
void funct(TrieNode *trie,char c[30])
{
    int i;
    sw=0;
    test(trie->pChildren[c[0] -97]);
    if(sw==1){
    for(i=0;i<=strlen(c)-1;i++)
        trie=trie->pChildren[c[i] -97];
    DisplayTrie2(trie);
    }
}
void DisplayTrie2(TrieNode *trie)
{
	...
	{   if (trie->NotLeaf != true)
		 ...
		 // display all the words stored through trie children
		for (count = 26; count >= 0; count--)
			    DisplayTrie2(trie->pChildren[count]);
	}
...
{
	TrieNode *trie;
	char UserInputWord[20], cont_insert=' ',c[30];
...
label_menu:
	while( option != 7)
	{
		...
		case 7:    //Exit
			  break;
		default:
			   cout << "Choose from the displayed options (1-7) !!!";
			   goto label_menu;
	  }//end switch
	}//end while
